...
from utils import QuestionSampler, read_data_file, compute_qald_metrics, convert_values_to_tensors, compute_qald_metrics_hybrid
import torch
...
def evaluate_ranking_loss(models, x_, y_, df_, loss_fn, mode='val', hybrid = False):
    """
    ...
    '''
    loader = DataLoader(dataset, batch_sampler=question_sampler, shuffle=False)
    for name, eval_model in models.items():
        ...
        with torch.no_grad():
            ...
            pred_dict[name] = pred_

    prec, recall, f1, df_output = compute_qald_metrics_hybrid(pred_dict, df_, gold_file_name=args.gold_file_name, topk=args.topk)
...
def main():
    ...
    # checkpoint_names = ['output/exp_lnn_lcquad_ensemble/{name}ensemble_with_type_best_model.pt' for name in names]
    models = {name : EnsembleTypeRule(args.alpha, -1, False) for name in names}
    ...
    for name in models:
        models[name].load_state_dict(torch.load(f'output/exp_lnn_lcquad_ensemble/{name}ensemble_with_type_best_model.pt'))
        models[name].eval()
    loss_fn = nn.MarginRankingLoss(margin=args.margin)
    avg_loss, f1, pred_, df_output = evaluate_ranking_loss(models, x_test, y_test, df_test, loss_fn, mode="test")
