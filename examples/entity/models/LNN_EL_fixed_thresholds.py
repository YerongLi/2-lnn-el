...
class PureNameLNN(nn.Module):
    ...
    def forward(self, x, mention_labels=None, debug=False):
        ...
        sim_features_ = torch.where(sim_features > self.threshold, sim_features, torch.zeros_like(sim_features))
...
class ContextLNN(nn.Module):
    ...
    def forward(self, x, mention_labels=None):
        ...
        sim_features = x[:, 0:4].view(-1, 4)
        sim_features_ = torch.where(sim_features > self.threshold, sim_features, torch.zeros_like(sim_features))
        disjunction_result = self.sim_disjunction_or(sim_features_ * context_mask)
...
class TypeLNN(nn.Module):
    def __init__(self, alpha, arity=-1, slack=None):
        ...
        self.threshold = torch.nn.Parameter(torch.Tensor([.5, .5, .5, .5]))
    ...
    def forward(self, x, mention_labels=None):
        ...
        # RULE 3: contains predicate
        type_features = x[:, 6].view(-1, 1)
        type_mask = type_features >= self.type_features_threshold

        sim_features = x[:, 0:4].view(-1, 4)
        sim_features_ = torch.where(sim_features > self.threshold, sim_features, torch.zeros_like(sim_features))
        disjunction_result = self.sim_disjunction_or(sim_features_ * type_mask)

        # RULE 1 + RULE 2 + RULE 3
        cat_features = torch.cat([lookup_features, disjunction_result, type_features], 1)
        yhat = self.predicate_and(cat_features)
        return yhat
