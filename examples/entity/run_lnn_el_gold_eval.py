...
# sys.path.append('../../../')
from utils import MyBatchSampler, tokblue, tokgreen, tokred
from torch.utils.data import DataLoader, TensorDataset
from utils import QuestionSampler, read_data_file, compute_qald_metricsList
import copy, os
import torch
import random, math
...
def evaluate_ranking_loss(eval_model, x_, y_, df_, m_labels_, ques_, loss_fn, mode='val'):
    ...
    with torch.no_grad():
        ...
        # print('Before')
        # prec, recall, f1, df = get_qald_metrics(pred_, m_labels_, ques_, args.gold_file_name, topk=args.topk)  # train and val both use 'val' mode
        prec, recall, f1, df = compute_qald_metricsList(pred_, df_, topk=args.topk)  # train and val both use 'val' mode
...
def train(model, train_data, val_data, test_data, checkpoint_name, num_epochs, margin, learning_rate):
    ...
    m_labels_train, ques_train = df_train.Mention_label.values, df_train.Question.values
    
    # x_val, y_val, m_labels_val, ques_val = val_data
    m_labels_val, ques_val = df_val.Mention_label.values, df_val.Question.values

    # x_test, y_test, m_labels_test, ques_test = test_data
    m_labels_test, ques_test = df_test.Mention_label.values, df_test.Question.values
    ...
    for epoch in range(num_epochs):
        ...
        # print(len(loader.dataset)) # valid
        for xb, yb in loader:
            ...
            idx += 1
            # if torch.isnan(loss): print(True, loss.item(),yb, yhat)
            total_loss += loss.item() * (yb.shape[0] - 1)
            loss.backward()
            optimizer.step()
        ...
        print("Epoch " + str(epoch) + ": "+ tokblue("avg train loss")+" -- " + str(avg_loss))
        #         print("Train -- loss is {}; F1 is {}".format(train_loss, train_f1))
        val_loss, (val_prec, val_rec, val_f1), val_pred, _  = evaluate_ranking_loss(model, x_val, y_val, df_val, m_labels_val, ques_val, loss_fn)
        print("Val loss {}; Prec {}; Recall {}; F1 {}".format(val_loss, val_prec, val_rec, tokgreen(str(val_f1))))
        test_loss, (test_prec, test_rec, test_f1), test_pred, df_output = \
            evaluate_ranking_loss(model, x_test, y_test, df_test, m_labels_test, ques_test, loss_fn, mode='test')
        ...
        if val_f1 >= best_val_f1:
            ...
            df_output.to_csv(args.output_file_name)
            best_test_loss, best_test_prec, best_test_rec, best_test_f1 = test_loss, test_prec, test_rec, test_f1
        log_str = "Best Test -- loss is {}; Prec {}; Recall {}; F1 {}".format(best_test_loss, best_test_prec, best_test_rec, tokred(str(best_test_f1)))
        print(log_str)
...
def main_predefined_split():
    ...
    args.checkpoint_name = "{}/{}.pt".format(output_file_folder, TYPE+ args.model_name + "_best_model")
    ...
    if args.dataset_name == "lcquad":
        ...
        df_train = pd.read_csv(f"./data/aida/blink_bert_box/{TYPE}_train.csv")
        df_valid = pd.read_csv(f"./data/aida/blink_bert_box/{TYPE}_testA.csv")
        df_test = pd.read_csv(f"./data/aida/blink_bert_box/{TYPE}_testB.csv")
        args.gold_file_name = "" 
    ...
    prec, recall, f1, df = compute_qald_metricsList(pred_, df_test, topk=args.topk, printerror=True)  # train and val both use 'val' mode
