...
python run_lnn_el_gold_webqsp.py \
--dataset_name webqsp \
...
--experiment_name exp_lnn_webqsp_pure_ctx_type_bert \
--num_epoch 30 \
--learning_rate 0.0001
...
--experiment_name exp_lnn_webqsp_pure_ctx_type_box \
--num_epoch 30 \
--learning_rate 0.0001
...
--dataset_name webqsp \
--model_name pure_ctx_type \
--experiment_name exp_logic_webqsp_pure_ctx_type \
--num_epoch 30 \
--learning_rate 0.0001 \
--topk 5
...
from models.lnn_el_websql import *


def get_qald_metrics(pred_, m_labels_, ques_, gold_file_name, topk=args.topk):
...
def train(model, train_data, val_data, test_data, checkpoint_name, num_epochs, margin, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        #         print("Train -- loss is {}; F1 is {}".format(train_loss, train_f1))
        val_loss, (val_prec, val_rec, val_f1), val_pred, _  = evaluate_ranking_loss(model, x_val, y_val, m_labels_val, ques_val, loss_fn)
        ...
        test_loss, (test_prec, test_rec, test_f1), test_pred, df_output = \
            evaluate_ranking_loss(model, x_test, y_test, m_labels_test, ques_test, loss_fn, mode='test')
...
def main_predefined_split():
    ...
    if args.dataset_name == "lcquad":
        ...
        df_train = pd.read_csv("./data/qald-9/train_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_valid = pd.read_csv("./data/qald-9/valid_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_test = pd.read_csv("./data/qald-9/test_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        args.gold_file_name = "qald/qald_data_gt.csv"
        ...
        # df_test = pd.read_csv("./data/lcquad/merge_gold_db_blink/test_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        # args.gold_file_name = "lcquad/lcquad_gt_5000.csv"
    elif args.dataset_name == "webqsp_bert":
        df_train = pd.read_csv("./data/webqsp/bert/train_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_valid = pd.read_csv("./data/webqsp/bert/valid_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_test = pd.read_csv("./data/webqsp/bert/test_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        args.gold_file_name = "webqsp/webqsp_data_gt.csv"
    elif args.dataset_name == "webqsp_box":
        df_train = pd.read_csv("./data/webqsp/box/train_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_valid = pd.read_csv("./data/webqsp/box/valid_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        df_test = pd.read_csv("./data/webqsp/box/test_gold.csv").sort_values(by=['QuestionMention', 'Label'])
        args.gold_file_name = "webqsp/webqsp_data_gt.csv"
    ...
    if args.model_name == "pure_ctx_type":
