...
class PureNameLNN(nn.Module):
    ...
    def forward(self, x, mention_labels=None, debug=False):
        ...
        jacc_features_ = torch.sign(jacc_features) * nn.ReLU()(torch.abs(jacc_features) - nn.Sigmoid()(self.threshold))
        # jacc_features_ = torch.sign(jacc_features) * nn.ReLU()(torch.abs(jacc_features) - self.threshold)
        feature_list.append(jacc_features_)

        # rule 2 (3) Lev(mention, label)
        lev_features = x[:, 2].view(-1, 1)
        lev_features_ = torch.sign(lev_features) * nn.ReLU()(torch.abs(lev_features) - nn.Sigmoid()(self.threshold))
        # lev_features_ = torch.sign(lev_features) * nn.ReLU()(torch.abs(lev_features) - self.threshold)
        feature_list.append(lev_features_)

        # rule 2 (4) Jaro(mention, label)
        jaro_features = x[:, 0].view(-1, 1)
        jaro_features_ = torch.sign(jaro_features) * nn.ReLU()(torch.abs(jaro_features) - nn.Sigmoid()(self.threshold))
        # jaro_features_ = torch.sign(jaro_features) * nn.ReLU()(torch.abs(jaro_features) - self.threshold)
        feature_list.append(jaro_features_)

        # rule 2 (5) spacy
        spacy_features = x[:, 3].view(-1, 1)
        spacy_features_ = torch.sign(spacy_features) * nn.ReLU()(torch.abs(spacy_features) - nn.Sigmoid()(self.threshold))
        # spacy_features_ = torch.sign(spacy_features) * nn.ReLU()(torch.abs(spacy_features) - self.threshold)
...
class ContextLNN(nn.Module):
    ...
    def forward(self, x, mention_labels=None):
        ...
        # context_features_ = torch.sign(context_features) * nn.ReLU()(torch.abs(context_features) - nn.Sigmoid()(self.context_threshold))
        ...
        jacc_features = x[:, 1].view(-1, 1)
        jacc_features_ = torch.sign(jacc_features) * nn.ReLU()(torch.abs(jacc_features) - nn.Sigmoid()(self.threshold))
        jacc_features_ *= torch.sign(context_features) * nn.ReLU()(torch.abs(context_features) - self.context_threshold)
        feature_list.append(jacc_features_)

        # rule 2 (3) Lev(mention, label)
        lev_features = x[:, 2].view(-1, 1)
        lev_features_ = torch.sign(lev_features) * nn.ReLU()(torch.abs(lev_features) - nn.Sigmoid()(self.threshold))
        lev_features_ *= torch.sign(context_features) * nn.ReLU()(torch.abs(context_features) - nn.Sigmoid()(self.context_threshold))
        # lev_features_ = torch.sign(lev_features) * nn.ReLU()(torch.abs(lev_features) - self.threshold)
        feature_list.append(lev_features_)
        ...
        jaro_features = x[:, 0].view(-1, 1)
        jaro_features_ = torch.sign(jaro_features) * nn.ReLU()(torch.abs(jaro_features) - nn.Sigmoid()(self.threshold))
        jaro_features_ *= torch.sign(context_features) * nn.ReLU()(torch.abs(context_features) - nn.Sigmoid()(self.context_threshold))
        # jaro_features_ = torch.sign(jaro_features) * nn.ReLU()(torch.abs(jaro_features) - self.threshold)
        feature_list.append(jaro_features_)
        ...
        spacy_features = x[:, 3].view(-1, 1)
        spacy_features_ = torch.sign(spacy_features) * nn.ReLU()(torch.abs(spacy_features) - nn.Sigmoid()(self.threshold))
        # spacy_features_ = spacy_features * nn.ReLU()(spacy_features - nn.Sigmoid()(self.threshold))
        # spacy_features_ = spacy_features * nn.Sigmoid()(spacy_features - nn.Sigmoid()(self.threshold))
        spacy_features_ *= torch.sign(context_features) * nn.ReLU()(torch.abs(context_features) - nn.Sigmoid()(self.context_threshold))
        # spacy_features_ = torch.sign(spacy_features) * nn.ReLU()(torch.abs(spacy_features) - self.threshold)
        feature_list.append(spacy_features_)
