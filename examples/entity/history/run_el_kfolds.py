...
parser.add_argument("--output_file_name", type=str, default="output/complex_kfolds.txt", help="checkpoint path")
...
def main():
    ...
    if args.use_kfolds:
        ...
        for fold_num in range(1, 6):
            ...
            output_file_name = "{}/complex_kfolds_{}.csv".format(output_file_folder, fold_num)
            Path(output_file_folder).mkdir(parents=True, exist_ok=True)
            args.output_file_name = output_file_name

            df_train = pd.read_csv("./data/type-qald-5-folds/split{}/train_fold.csv".format(fold_num))
            df_test = pd.read_csv("./data/type-qald-5-folds/split{}/test_fold.csv".format(fold_num))
        ...
        with open("output_w_spacy.txt", 'a') as f:
            f.write("model={}; use_fixed_threshold={}; alpha={}; p={}; r={}; f1={}; lr={}; margin={}\n".format(args.model_name,
                                                                                                      args.use_fixed_threshold,
                                                                                                      args.alpha,
                                                                                                      best_scores[
                                                                                                          'precision'],
...
if __name__ == '__main__':
    ...
    args.experiment_name = "exp_sigmoid_without_missing_0812"
    for model_name in ['complex']:
