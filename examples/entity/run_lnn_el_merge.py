...
from utils import QuestionSampler, read_and_filter_lcquad, compute_qald_metrics, convert_values_to_tensors
import torch
from torch import nn, optim
from pathlib import Path
from torch.utils.tensorboard import SummaryWriter
from models.LNN_EL_merge import ComplexRuleDbpediaBLINK, EnsembleRule
import pandas as pd
# random.seed(103)
...
def train(model, train_data, val_data, test_data, checkpoint_name, num_epochs, margin, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, val_f1, val_pred, df_output = evaluate_ranking_loss(model, x_val, y_val, df_val, loss_fn, mode='val')
        ...
        if val_f1 >= best_val_f1:
            ...
            df_output.to_csv("ensembleComplexOutput.csv")
        print("Current val -- loss is {}; F1 is {}".format(val_loss, val_f1))
        print("Best    Val --  best loss (so far) is {}; F1 is {}".format(best_val_loss, best_val_f1))

        # test_loss, test_f1, test_pred = evaluate_ranking_loss(model, x_test, y_test, df_test, loss_fn, mode='test')
        # print("Current Test -- loss is {}; F1 is {}".format(test_loss, test_f1))

    # TODO: after training performance
    show_current_performance()
...
def evaluate_ranking_loss(eval_model, x_, y_, df_, loss_fn, mode='val'):
    ...
    with torch.no_grad():
        ...
        print("Current Val -- prec, recall, f1, loss", prec, recall, f1, avg_loss)

    return avg_loss, f1, pred_, df_output


def main():
    """Runs an experiment on pre-defined train/test split"""

    average_performance = []
    fold_num = 'predefined'
    output_file_folder = "output/{}".format(args.experiment_name)
    output_file_name = "{}/lnnel_merge_{}.csv".format(output_file_folder, fold_num)
    Path(output_file_folder).mkdir(parents=True, exist_ok=True)
    args.output_file_name = output_file_name

    # read lcquad merged data
    # TODO: remove the hacking part .head(1000)
    df_train = pd.read_csv("./data/lcquad/merge_db_blink/train.csv")
    # df_train = pd.read_csv("sampleNBAquestion.csv")
    df_test = pd.read_csv("./data/lcquad/merge_db_blink/test.csv")
    train_data, test_data = read_and_filter_lcquad(df_train, df_test, device)
    ...
    # training
    train(model, train_data, test_data, test_data, args.checkpoint_name, args.num_epoch, args.margin, args.learning_rate)
