...
from utils import QuestionSampler, tokblue, tokgreen, tokblue, tokred, read_data_file, compute_qald_metrics, compute_qald_metricsList, compute_long_metrics, convert_values_to_tensors
import torch
...
def compute_loss(model, df, loader, loss_fn, optimizer=None):
    ...
    for x_0, y_0, qm_0, db_mask0, blink_mask0 in loader:
        ...
        # print('x_', list(x_.shape))
        # print('y_', list(y_.shape))
        # print('x_', x_)
        # print('y_', y_)
        # print('qm_', qm_)
...
def train(model, train_data, val_data, test_data, checkpoint_name, num_epochs, margin, learning_rate, is_long = False):
    ...
    for epoch in range(num_epochs):
        model.train()
        train_loss, batch_length = compute_loss(model, df_train, loader, loss_fn, optimizer)

        # show status after each epoch
        print("Epoch " + str(epoch) + ' / ' + str(num_epochs) + ": " + tokblue("avg train loss -- ") + str(train_loss))
        ...
        test_loss, test_f1, test_pred, df_output = \
            evaluate_ranking_loss(model, x_test, y_test, df_test, loss_fn, mode='test', is_long=is_long)
        ...
        print("Best Val --  best loss {}; F1 {}".format(best_val_loss, tokred(str(best_val_f1))))
...
def evaluate_ranking_loss(eval_model, x_, y_, df_, loss_fn, mode='val', printerror=False):
    """
    ...
    with torch.no_grad():
        ...
        # if is_long:
        #     prec, recall, f1, df_output = compute_long_metrics(pred_, df_, gold_file_name=args.gold_file_name, topk=args.topk, is_long = is_long)
        # else:
        prec, recall, f1, df_output = compute_qald_metricsList(pred_, df_, topk=args.topk, printerror=printerror)
        # prec, recall, f1, df_output = compute_qald_metrics(pred_, df_, gold_file_name=args.gold_file_name, topk=args.topk, is_long = is_long)
        
        # avg_loss, _ = compute_loss(eval_model, loader, loss_fn, optimizer=None)
        avg_loss = 0
        print("Current {} -- prec {}; recall {}; f1 {}; loss {}".format(mode, prec, recall,tokgreen(str(f1)), avg_loss))
...
def main():
    ...
    if args.dataset_name == "lcquad":
        ...
        if equalization:
            ...
            if TYPE is not None:
                ...
                li = [
                    pd.read_csv("./data/lcquad/blink_bert_box//full_train_gold.csv"),
                    pd.read_csv("./data/lcquad/blink_bert_box//full_valid_gold.csv"),
                ]              
        ...
        # df_test = pd.read_csv("./data/lcquad/blink_bert_box//test_gold.csv")
        if TYPE is not None:
            df_test = pd.read_csv(f"./data/lcquad/blink_bert_box//{TYPE}_test.csv")
        ...
        args.gold_file_name = "./data/aida/full_aida_gt_5000.csv" 
    ...
    if args.use_type:
        if args.dataset_name == "webqsp":
            ...
            if 'blink_box_bert' in args.experiment_name:
                ...
                args.model_name = 'ensemble_with_type'
                model = EnsembleTypeRule(args.alpha, -1, False)
    else:
        args.model_name = 'ensemble_without_type'
    ...
    if TYPE is not None:
        args.checkpoint_name = f'output/exp_lnn_lcquad_ensemble/{TYPE}{args.model_name}_best_model.pt'
    else:
        args.checkpoint_name = f'output/exp_lnn_lcquad_ensemble/full{args.model_name}_best_model.pt'
    # args.checkpoint_name = f'output/exp_lnn_lcquad_ensemble/full{args.model_name}_best_model.pt'  
    # args.checkpoint_name = 'output/exp_lnn_lcquad_ensemble/fullensemble_without_type_best_model.pt'
    # args.checkpoint_name = '/home/yerong/Downloads/Locationensemble_with_type_best_model.pt.7'
    
    # args.checkpoint_name = 'output/exp_lnn_lcquad_ensemble/PersonOrganizationensemble_with_type_best_model.pt'
    ...
    # train(model, train_data, valid_data, test_data, args.checkpoint_name, args.num_epoch, args.margin, args.learning_rate, is_long)
    ...
    avg_loss, f1, pred_, df_output = evaluate_ranking_loss(model, x_test, y_test, df_test, loss_fn, mode="test",printerror=True)
