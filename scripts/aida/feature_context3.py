...
with open('key.txt', 'w') as f:
	for entry in document_list:
		s = [k[0].lower() for k in entry['mention']]
...
feature_list = list(features.find({'t_': 'c3'}))
...
def mentionOverlap(docEntity):
	...
	if (doc, ent, label) in known_feature_set: return
	lbl = label.replace(' ', '_')
	try:
		...
		lcand = label.lower()
		for m in context:
			if m.find(lmention) != -1:
				first = m
				break
		if first is None: 
			score = 0.0
		else:
			score = Features.lev.get_sim_score(m,lcand)
		feature_dict.append({'t_': 'c3', 'd': doc, 'men': ent, 'cand': label, 's_': score})
