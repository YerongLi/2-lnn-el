...
json_list = list(features.find({'t_' : 'c3'}))
djson = dict()
...
for pos, dataset in enumerate(datasets):
	...
	def process(i):
		try:
			...
			doc = df.iloc[i].Question.split(' ')[0]
	...
	# print(data)
	new_df = pd.DataFrame(data, columns=['Question','Mention_label','Features','Label','Mention','QuestionMention','db','blink'])
