...
class LNN:
    ...
    def upward(self, source=None, func='upward', **kwargs):
        ...
        node_slice = kwargs.get('node_slice', (0, len(nodes)))
        nodes = nodes if node_slice is None else nodes[slice(*node_slice)]
        progress_inference = tqdm(total=len(nodes), initial=node_slice[0],  
                                  file=kwargs.get('file'), 
                                  disable=not kwargs.get('progress_inference',False))
        for node in nodes:
            val = getattr(node, func)(**kwargs) if hasattr(node, func) else None
            diff += val if val is not None else torch.tensor([0, 0.])
            progress_inference.set_description(func+'_'+kwargs.get('steps','')+', '+str(diff.detach().round()))
    ...
    def propagate(self, **kwargs):
        ...
        inference = {'both' : [self.upward, self.downward], 'upward': [self.upward], 'downward': [self.downward], 'reverse':[self.downward, self.upward]}
        while not(converged):
            diff = 0.
            for infer in inference[kwargs.get('direction','both')]: # TODO: arrest inference at contradiction
    ...
    def train(self, epochs=1e3, print_epoch=None, **kwargs):
        ...
        while (not weights_converged) and (epoch <= epochs):# and not(inference_converged):
            ...
            loss = torch.dot(loss_terms, kwargs.get('coefficients', torch.tensor([1., 1.]))) # coefficients [unsupervised, supervised]
            if print_: 
...
class Node:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self, 'lnn'):  # inherit lnn from operands
            for op in args:
...
#                 print('nomap', self, op, self.opvarmap[op_index], self.vars)
        ...
        coverage = {}
        for op_index, op in enumerate(self.operands):  # check var disjointness across ops
            for slot in self.opvarmap[op_index]:
                coverage.setdefault(slot, 0)
                coverage[slot] += 1
...
class Formula(Node):
    ...
    @property
    def alpha(self): return self.lnn.alpha
    ...
    def aggregate(self, key, new_bounds):  
        ...
        grounding = self.lnn.grounding(key)
#         if grounding not in self.bounds:  # not exist
#             if isinstance(self, Pred):  # don't store default/unk Pred
#                 default = self.default()
#                 if ((new_bounds[0] == default[0] and new_bounds[1] == default[1]) or 
#                     (new_bounds[0] == 0 and new_bounds[1] == 1)):  # default or unknown
#                     return 0.
        prev_bounds = self[grounding] + 0
        if grounding in self.bounds and has_grad(prev_bounds) and (
            (new_bounds == prev_bounds).prod() or (new_bounds == Formula.unknown()).prod()) :  # exists or unknown
            return 0.
#         bounds = torch.stack((torch.max(prev_bounds[0], new_bounds[0]),
#                               torch.min(prev_bounds[1], new_bounds[1])))  # max/min pref. last
#         self.bounds[grounding] = bounds
#         if bounds[0] > bounds[1]:
#             self.contra.setdefault(grounding, 0)
#             self.contra[grounding] += 1
#         print('[Aggregate]', self, self.args, self.lnn.grounding_str(grounding), prev_bounds, new_bounds, bounds)
    ...
    def groundings(self, bindings=None):  # retrieve node's groundings under bindings
        ...
        for key in self.bounds:
            for i, const_id in enumerate(key):
                if isinstance(bindings, tuple):  # binding is direct grounding
                    if bindings[i] is not None and const_id != bindings[i]:
                        break
                elif i in bindings and bindings[i] is not None:
    ...
    def inference_groundings(self, bindings=None):  # [0, 1, 2, 3] -> [0, 1], [1, 2, 3]
        ''' Determine operator/operand(s) grounding cover under bindings '''
        
        
        # inference_groundings not used by quantifier, but just for cases where opvars=vars
        bindings = bindings if bindings else self.bindings
        bindmaps = [[None for _ in self.opvars]]
        freevar = {i for i in range(len(self.opvars))}
        for i in bindings:
            if len(bindings[i]):
                freevar -= {i}
            newbindmaps = []
        ...
        freepart = []
        for i in range((0 if self.disjoint else 1), len(freevar)+1):  # length of free var partial
            freepart.extend(itertools.combinations(freevar, i))
        
        yielded = {}
        for grounding in self.groundings(bindings):
            if grounding not in yielded:
                yielded[grounding] = True
                yield grounding
        ...
        for op_index, op in enumerate(self.operands):
            groundings = set()
            for bindmap in bindmaps:
                for partial in op.groundings(self.map(op_index, bindmap)):
                    if partial is False:
                        continue
                    pattern = self.mapvar(op_index, partial, bindmap)
                    if pattern is False:
                        continue
                    if None not in pattern:
                        groundings |= {pattern}
                    else:
                        for part in freepart:
                            hascontent = False
                            defined = True
                            sub = tuple()
                            pattern_index = 0
                            for p in part:
                                while pattern_index < p:
                                    if pattern[pattern_index] is not None:
                                        hascontent = True
                                    pattern_index += 1
                                pattern_index += 1
                                sub += (pattern[p],)
                            ...
                            while pattern_index < len(pattern):
                                if pattern[pattern_index] is not None:
                                    hascontent = True
                                pattern_index += 1
                            if defined and hascontent:
                                partmatch.setdefault((part, sub), []).append(pattern)
        ...
        while len(partmatch):
            newpartmatch = {}
            for psub in partmatch:
                tojoin = set(partmatch[psub])
                part, sub = psub
                for pattern1 in tojoin:
                    for pattern2 in tojoin:
                        ...
                        newsub = tuple()
                        complete = True
                        for i, p1, p2 in zip(range(len(pattern1)), pattern1, pattern2):
                            if p1 is None and p2 is None:
                                newpattern.append(None)
                            elif p1 is None and p2 is not None:
                                newpattern.append(p2)
                                if i not in part:  # not a join var
                                    newpart += (i,)
                                    newsub += (p2,)
                            elif p1 is not None and p2 is None:
                                newpattern.append(p1)
                                if i not in part:  # not a join var
                                    newpart += (i,)
                                    newsub += (p1,)
                            elif p1 == p2:
                        ...
                        if complete:
                            ...
                            if None not in newpattern:
                                if newpattern not in yielded:
                                    yielded[newpattern] = True
                                    yield newpattern
                            else:
                                toadd = True
                                if len(newpart) and newpart in freepart:  # apply restriction
                                    toadd = not (newpart, newsub) in partmatch
                                if toadd:
                                    for npart in freepart:
                                        defined = True
                                        nsub = tuple()
                                        for p in npart:
                                            nsub += (newpattern[p],)
                                            if newpattern[p] is None:
                                                defined = False
                                                break
                                        if defined:
                                            newpartmatch.setdefault((npart, nsub), []).append(newpattern)
            partmatch = newpartmatch
    ...
    def state(self, key=(), shorthand=False, bounds=None):
        ...
        if (bounds[0] <= 1-self.alpha) and (bounds[1] >= self.alpha):
            ...
            return '~U' if shorthand else '~Unknown'
        else: raise Exception('out of bounds:', bounds, 'alpha', self.alpha)
    
    def print_graph(self, **kwargs):
        '''print states of all nodes in graph'''
    ...
    def reset_bounds(self, **kwargs): # TODO: should only be the logic-cone of changed node (graph partitioning and inference counter), to reset only downstream from weight changes
        def reset():
            if self.init_bounds is None: self.init_bounds = self.bounds.copy()
            else: self.bounds = self.init_bounds.copy()
                
        if kwargs.get('reset_facts',False): 
            if type(self) == Pred: reset() # reset only the facts
        else: reset() # reset all bounds
...
class Neuron(Formula):
    ...
    def __init__(self, *args, **kwargs):
        ...
        min_alpha = self.weights.sum()/(self.weights.sum()+self.weights.max())
        if self.lnn.alpha <= min_alpha:
            self.lnn.alpha = min_alpha+1e-10
#             print('alpha updated {:4e}'.format(self.lnn.alpha))
        
    def input_bounds(self, grounding):  # stack input bounds together
        bounds = ()
        for op_index, op in enumerate(self.operands):
            op_bounds = op[self.map(op_index, grounding)]
#             if self.state(bounds=op_bounds) == 'Contradiction': # don't propagate contradiction
#                 return False
            bounds += (op_bounds,)
        return torch.stack(bounds, dim=1)
    
#     in_contradiction = lambda self, bounds: self.state(bounds=bounds) == 'Contradiction' 
#     has_contradiction = lambda self, bounds: torch.tensor([self.in_contradiction(bounds=b) for b in bounds]).prod()
...
#             if self.has_contradiction(input_bounds):
#                 self.lnn.has_contradictions = True
#                 continue
...
#                 continue
            ...
            for op_index, op in enumerate(self.operands):
                op_bounds = alpha_scaled(new_bounds[:, op_index], self.alpha, op.alpha)
                diff += op.aggregate(self.map(op_index, grounding), op_bounds)
#                 if self.in_contradiction(op_bounds): 
#                     self.lnn.has_contradictions = True
#                     break
        return diff
        ...
        if has_grad(self.weights, fn=False):
            ...
            with torch.no_grad(): 
                if (self.weights.grad.min()==self.weights.grad.max()) or (self.weights == 0).sum():
                    self.weights = (self.weights-self.weights.grad* lr).clamp(min=0) # don't scale gradients when zero weights exists or gradients drop simulataneously
                else:
                    scaled_grad = self.weights.grad/self.weights.grad.abs().max() # extra gradient optimisation
                    self.weights = (self.weights-scaled_grad* lr) # only scale gradients when different directions exist
        ...
        for grounding in op.groundings(self.op_bindings(0)):
            ...
            for i, const in enumerate(grounding):  # split quantified/non-quantified variables
                if self.opvars[i] in self.vars:
                    ...
                    val += (const,)
            bounds = op[grounding]
#             if self.state(bounds=bounds) == 'Contradiction': return 0.
    ...
    def downward(self, **kwargs):
        ...
        for grounding in list(op.groundings(self.op_bindings(0))):
            ...
            for i, const in enumerate(grounding):  # split quantified/non-quantified variables
                if self.opvars[i] in self.vars:
                    key += (const,)
#             if self.state(bounds=self[key]) == 'Contradiction': return 0.
...
def tailored_piecewise(x, alpha, weights, inverse=False):
    ...
    if inverse:
        ...
        if ((0 <= x) * (x <= Xf)).sum():
            ...
            Gm = (Yt - Yf) / (Xt - Xf)
            result += (Xf < x) * (x < Xt) * ((Yf + (x - Xf) * Gm) if (Xf!=Xt) else torch.zeros(1)) # else at strictly classical
        elif ((Xt <= x) * (x <= beta)).sum():
            Gt = (1 - Yt) / (beta - Xt)
            result += (Xt <= x) * (x <= beta) * ((((x - Xt) * Gt) if (Xt!=beta) else torch.zeros(1)) + Yt) # else at perfect rules
    return result
...
def and_inv_tailored(out_bounds, bounds, alpha, weights, mode=1):  # tailored activation functional inverse
    ...
    if bounds.shape[1] > 2:
        ...
        partial_weights = torch.cat([unpack_partial(weights[None,:],w) for w in range(weights.shape[0])]) # TODO: vectorise
        partial_and = and_(mode)(partial_bounds, alpha, partial_weights)
    elif bounds.shape[1] == 2:
        partial_and = bounds.flip(dims=(1,))
    else: raise Exception('incorrect bounds shape', bounds.shape)
    stack = implies_(mode)(torch.stack([partial_and, out_bounds.repeat(partial_and.shape[1],1).t()]).permute(2,1,0),alpha, torch.ones(partial_and.shape[1], 2))
    result = divisor_fill(divident=val_clamp(stack), divisor=weights, fill=1.) # fill to make logical inverse True
    non_classical = (partial_and[1,:]<=1-alpha).float()*(out_bounds[1].repeat(1,partial_and.shape[1])<=1-alpha).float() # flags for non classical points of logical inverse
    result = torch.where(non_classical.repeat(2,1)==True, tracked_unknown(result), result) # classical equality does not hold
    result[0,:] = torch.where((result[0,:]>1-alpha).float()*(result[0,:] > f_inv[0,:]).float()==True, f_inv[0,:], result[0,:]) # lower bound corrections
    result[1,:] = torch.where((result[1,:]<alpha).float()*(result[1,:] < f_inv[1,:]).float()==True, f_inv[1,:], result[1,:]) # upper bound corrections
    if has_nan(result): raise Exception('Nans detected', result) # for debugging purposes
    return result
...
def and_f_inv_tailored(out_bounds, bounds, alpha, weights):  # tailored functional inverse
    if (weights == 0).prod(): # bypass node
        ...
        f_inv = divisor_fill(divident=f_inv, divisor=weights, fill=weights.sum()) # fill to make the f_inv True where weights are zero
    return  torch.clamp(f_inv, max=1)
...
def implies_tailored(bounds, alpha, weights, mode=1):
    ...
    stack = torch.stack([bounds[:,:,0], not_(bounds[:,:,1],transpose=True)]).permute(1,2,0) # negate and stack dims: 0=batch, 1=[lower,upper], 2=[lhs,rhs]
...
val_clamp = lambda x, min_=0, max_=1, grad=False: x - (x.detach() - min_).clamp(max=0) - (x.detach() - max_).clamp(0) if grad else x.clamp(0,1.) # gradient-transparent clamp
has_grad = lambda tensor, fn=True: True if (tensor.grad_fn is not None and fn) else (True if tensor.grad is not None else False)
has_nan = lambda tensor: (torch.stack([torch.isnan(tensor.flatten()[i]) for i in range(tensor.numel())])).sum()
tracked_unknown = lambda tensor1, tensor2=Formula.unknown(): torch.stack((torch.min(tensor1[0], tensor2[0]), 
                                                                         torch.max(tensor1[1], tensor2[1])))
