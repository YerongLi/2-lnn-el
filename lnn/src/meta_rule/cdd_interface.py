...
class cdd_lnn(nn.Module):
    def __init__(self, alpha, arity, with_slack):
        ...
        if np.sum(is_point) > 0:
            ...
            self.gamma = nn.Parameter(torch.zeros(1, self.points.shape[0]).uniform_(0, 1))
            
        self.has_ray = False
        if np.sum(~is_point) > 0:
            self.has_ray = True
            self.rays = torch.as_tensor(rays_and_points[~is_point,], dtype=torch.float)
            self.mu = nn.Parameter(torch.zeros(1, self.rays.shape[0]).uniform_(0, 1))
    ...
    def forward(self):
        ...
        if self.has_ray:
            rays_weights = func.softplus(self.mu)
        ...
        if self.with_slack:
            ...
            return params[0], params[1:self.arity + 1]
    ...
    def make_constraints_with_slacks(self, alpha, arity):
        ...
        A = np.append(np.zeros((2*arity, 1)), np.diag(-np.ones(2*arity)), axis=1) 
        
        #equation (1*)/(3*) in LNN draft
        equ_1_star_lhs = np.append(np.ones((arity, 1)), \
                                   np.append(np.diag(np.full((arity), -alpha)), np.diag(np.full((arity), -1)), axis=1), \
                                   axis=1)
        A = np.append(A, equ_1_star_lhs, axis=0)
        
        #equation (2)/(4) in LNN draft
        equ_2_lhs = np.reshape(np.append(np.append(-1, np.full((arity), 1-alpha)), np.zeros((arity))), \
                               (1,-1))
        A = np.append(A, equ_2_lhs, axis=0)
